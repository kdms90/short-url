var $certification = {
    _retrive: function ($url, $consultant_id, $page = 1, $parentDiv = '') {
        ...
        $.ajax({
            ...
            success: function (response, textStatus, jqXHR) {
                $('#added-certifications-table').find('tbody').html(response.entities);
    ...
    handle: function () {
        var $modal = $('#app-modal');
        //Récurapation de la liste des certification
        $(document).on('click', 'a.certifications', function (e) {
            e.preventDefault();
            var $parentDiv = $('#app');
            var $url = $(this).attr('data-action');
            var $page = $(this).attr('data-page');
            var $consultant_id = parseInt($(this).attr('data-consultant'));
            $certification._retrive($url, $consultant_id, $page, $parentDiv)
        });
        //Récupération du formulaire
        $(document).on('click', '.add-certification-line', function (e) {
            e.preventDefault();
            var $parentDiv = $('#app');
            var $url = $(this).attr('data-action');
        ...
        $(document).on('submit', '#app-modal form.consultantCertificationForm', function (e) {
            e.preventDefault();
            ...
            $form.ajaxSubmit({
                success: function (response) {
                    ...
                    if (response.status === 200) {
                        $('#added-certifications-table').find('tbody').html(response.entities);
                        foundation.showPrompt('Certification', 'Certification ajoutée avec succès');
                        $form.fadeOut('slow');
                        $modal.modal('toggle');
                    }
        ...
        $(document).on('click', '#certifications-lines .delete-certification-line', function (e) {
            e.preventDefault();
            var $parentDiv = $('#app');
            var $url = $(this).attr('data-action');
            var $data = '';
